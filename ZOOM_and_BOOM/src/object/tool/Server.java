...
public class Server extends JFrame {
	...
	public Server(int portNum) {
		...
		frame.addWindowListener(new java.awt.event.WindowAdapter() {
			...
			public void windowClosing(java.awt.event.WindowEvent windowEvent) {
				if (JOptionPane.showConfirmDialog(frame, "Are you sure you want to close the server?",
	...
	public void runForever() {
		...
		while (true) {
			...
			// try to accept connect request of client and create connection
			// channel
			try {
		...
		public void run() {
			...
			while (true) {
				try {
					...
					if (message instanceof Player) {
						...
						case "frameEnd":
							...
							if (checkAnswer(image, startX, startY, width, height))
								...
								objOut.writeObject("Wrong");
							break;

						// request of attack another players
						case "attack":
							// receive attack information
							sourceName = (String) objIn.readObject();
							System.out.println("attacker: " + sourceName);
							String targetName = (String) objIn.readObject();
							int color = (int) objIn.readObject();
							System.out.println("Let the victim know");
							// attack
						...
						case "picName":
							...
							//Splash splash = (Splash) objIn.readObject();
							ArrayList<Splash> splash= (ArrayList<Splash>)objIn.readObject();
							System.out.println(splash);
							System.out.println("aaa: " + sourceName);
							for (ConnectionThread connection : connections) {
								if (sourceName.equals(connection.name)) {
									System.out.println("Attacker show victim's image!");
						...
						case "updateMoney":
							...
							int money = (int) objIn.readObject();
					...
					/*Bugggggggggggggggggggggggggggggggggggggggggggggggggggggg
					else if (message instanceof Image) {
						System.out.println("WWWWW");
						Image img = (Image)objIn.readObject();
						for (ConnectionThread connection : connections) {
							if (sourceName.equals(connection.name)) {
								System.out.println("###Attacker show victim's image!");
								connection.send(img);
